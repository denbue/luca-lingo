...
export const exportDictionaryAsText = (data: DictionaryData) => {
  ...
  if (data.title) {
    ...
    content += `DICTIONARY_TITLE_TRANSLATION: \n`;
  }
  content += '\n';
  ...
  if (data.description) {
    ...
    content += `DICTIONARY_DESCRIPTION_TRANSLATION: \n`;
  }
  content += '\n';
  ...
  data.entries.forEach((entry, index) => {
    ...
    if (entry.origin) {
      ...
      content += `ENTRY_${index + 1}_ORIGIN_TRANSLATION: \n`;
    }
    ...
    entry.definitions.forEach((def, defIndex) => {
      ...
      if (def.example) {
        ...
        content += `ENTRY_${index + 1}_DEF_${defNum}_EXAMPLE_TRANSLATION: \n`;
      }
    });
    content += '\n';
  });
